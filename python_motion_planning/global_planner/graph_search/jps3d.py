...
class JPS3D(AStar3D):
    ...
    def getJpsSucc(self, node: Node3D):
        ...
        for dev in range(num_neib + num_fneib):
            if dev < num_neib:
                ...
                new_node = self.jump(node, Node3D((dx, dy, dz), None, sqrt(dx ** 2 + dy ** 2 + dz ** 2), None, self.weight))
                if new_node is None:
                    continue
            else:
    ...
    def jump(self, node: Node3D, motion: Node3D) -> Node3D:
        """
        ...
        for k in range(num_neib):
            ...
            if self.jump(new_node, new_motion):
                return new_node
